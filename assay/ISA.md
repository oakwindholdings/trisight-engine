...
- [x] ISC-151: All work pushed to origin (assay-phase1 branch) after every milestone commit — `git log origin/assay-phase1` proves it
- [x] ISC-152: Discovery record per sealed strategy: name, seal ref, claim source file:line, realized-record source, window — or documented NOT FOUND
- [x] ISC-153: Oakwind disambiguation recorded (Swing Trader vs Investor Daily) with the evidence for "most likely correct"
- [x] ISC-154: Phase-2 code lives outside COMPUTE_ROOTS; gate verify-store stays green (zero stale seals) throughout
- [x] ISC-155: PredecessorClaim records: content-addressed, provenance = exact file path + excerpt, read-only ingestion (nothing written back — grep proves no writes outside assay/)
- [x] ISC-156: PredecessorRealized records: same discipline, from fills ledgers/paper reports
- [x] ISC-157: InflationFactor computed only when claim and realized windows overlap comparably; mismatch → Refused('window_mismatch' via invalid_params detail), never a stretched ratio
- [x] ISC-158: Every strategy without a computable ratio gets an explicit refusal record naming what was missing (the PRD predicts ~5/9 irreproducible — that finding IS a deliverable)
- [x] ISC-159: Estate inflation distribution report generated FROM records (display path re-derives nothing)
- [x] ISC-160: Known fleet-wide survivorship-bias finding (frozen universe pools, workflow w4nfwu675) cited in every per-strategy report as a structural inflator
- [x] ISC-161: Phase-3 candidates ALL pre-registered before any evaluation (registration timestamps < first Run timestamp — provable from records)
- [x] ISC-162: Every candidate evaluated under identical frictions/window; identical params_hash structure across the sweep
- [x] ISC-163: Every candidate gets an adversary pass; survivors = verified:true receipts only
- [x] ISC-164: Sweep report discloses the full candidate count and failure distribution — no silent survivorship in the survivor report (the tool must not commit the sin it measures)
- [x] ISC-165: Anti: predecessor estate untouched — `git -C <estate repos> status` clean for all four peer repos at campaign end
...
- 2026-08-18 — DESIGN REVIEW ROUND (Bob asked the right question: "has this plan itself been reviewed?" — it had not, repeating the exact gap the Changelog condemns; withdrawn from ratification). Cato adversarial: REJECT as written (7 critical — led by the git_commit fixed-point impossibility, self-asserted epochs, the cli.ts laundering vector, review_ref theater, no revocation, non-recursive code hash). Forge peer: SOUND-WITH-CHANGES (4 critical — including loadUniverse's membership-vs-availability gap that would refuse every honest rotation backtest, the missing delisting policy, replay hard-breaking at the first epoch transition, and grammar_version silently invalidating all 136 spec hashes with an exact undefined-omission fix). Both converged on: epochs computed-from-git (tree hashes, historical re-derivation), verifier code inside the sealed hash, recursive all-file hashing, content-addressed diff-bound reviews. Design v2 written with every finding dispositioned by ID (design/GRAMMAR-EXTENSION-MECHANISM.md); verdicts preserved in design/REVIEWS-2026-08-18.md; v1 in git history per I7. Earnings answer changed to Cato's measurement design (perturbation dual-run + mandatory spread slice). v2 sits at Bob's ratification line with four reserved decisions.
- 2026-08-18 — REMOTE MIGRATION PENDING: Bob directed all work to https://github.com/oakwindholdings/trisight-engine as the canonical remote. Remote `oakwind` is configured and the repo exists (empty, readable), but BOTH local GitHub identities are denied write (bob-stewart: 403; exochain: push=false — verified via gh api, active account restored to bob-stewart). Blocked on a GitHub-side grant only; everything remains safely pushed to origin (bob-stewart/trisight) meanwhile. The moment access exists: push main + assay-phase1 + worktree branch, mirror historical branches, set upstreams to oakwind.
- 2026-08-18 — REMOTE MIGRATION COMPLETE: canonical remote is now https://github.com/oakwindholdings/trisight-engine (25 branches: main, assay-phase1, the working branch, and 22 historical). Blocked twice en route, both times correctly: first by missing write access, then by GitHub push protection catching a live Anthropic API key (.vscode/settings.json, in history since c3144e71 and present at HEAD — the predecessor's hygiene one final time). Key revoked by Bob, tip sanitized (20e12fb), push allowed via the documented revoked-secret flow. History preserved intact per I7 — the mistake stays visible, revoked and disclosed, not rewritten away. Upstreams now track oakwind; origin (bob-stewart/trisight) retained as a secondary.

## Decisions (Campaign 3 — build phase, waves W0-W4)

- 2026-08-18 — BOB'S RULINGS on design v2: (1) Epoch model RATIFIED. (2) Earnings measurement design RATIFIED. (3) Rotation universe semantics — question returned for plain-English explanation; PAI's evidence-based read is SCREEN (estate universes were home-built filter caches over vendor listings, not licensed index rosters); awaiting Bob's one-word confirmation, gates only W3. (4) Wave order AS PROPOSED (W0→W4).
- 2026-08-18 — SEQUENCING: W0 changes kernelCodeHash itself, which the current gate reads as universal seal staleness; per design F22, W0+W1 land as ONE epoch-declaring commit — genesis Epoch-1 (retroactive, blesses all pre-epoch history at code_hash sha256:5bde7122…) and Epoch-2 (the W0+W1 tree) declared atomically. Fixed-point note: Epoch tree identity is defined over the COMPUTE_ROOTS subtrees only (store objects live outside them), so declaring an epoch cannot change the tree identity it declares — C1's fix, one level deeper.
- 2026-08-18 — RULING #3 RESOLVED: SCREEN (Bob confirmed: "screen of active symbols"). Massive reference/delisted endpoints suffice; no index-constituent data needed. AMENDMENT to W3 scope, per Bob's insight that the screen is itself a key lambda input: universes are no longer ingested artifacts but COMPUTED records — a pre-registered, hashed UniverseSpec (screen definition) evaluated point-in-time against ingested reference FACTS produces each universe series via its own hash-in/hash-out invocation with trace and receipt. Screen parameters join the sweep space (screen variants × strategy variants, all pre-registered); a mandatory perturbed-screen adversarial slice exposes universe-definition overfitting. The trisight-trader streamlined universes become the first registered incumbent screens (read-only discovery → grammar translation → registration). Facts ingested, claims computed. W3 build gets its own review per the epoch process.
- 2026-08-18 — W0+W1 COMPLETE, THE EPOCH MECHANISM IS LIVE. Chain: Epoch-1 (genesis, retroactive, blesses an ENUMERATED 381-record pre-epoch set, contentHash sha256:143c1164…) → Epoch-2 (W0+W1 tree) → Epoch-3 (head). Cato reviewed across FOUR passes: changes-required (X1-X14: the bridge was declared-not-derived, could bless two histories, verified-suppression unimplemented, plus 11 more) → all blocking+major items landed (enumerated blessing, genesis-only retroactivity, honesty_flags aggregation rule, clean-tree gate step, head-anchored replay, tracked+authorship-checked reviews, --store executability, byte-faithful single-owner framing, compile-time seal partition) → approve-for-epoch → two delta approvals. THE GATE CAUGHT WHAT BOTH REVIEWERS AND THE RATIFIED DESIGN MISSED: pre-epoch traces carry the retired algorithm's hash no epoch can match — surfaced as exit-5 AFTER E2's declaration, forcing the fix through the mechanism's own full discipline (Epoch-3, its own review). Cato's record: "Two reviewers and a ratified design missed it; the gate did not." Red-first evidence: live unstaged-kernel-edit gate failure (evidence/17), 11 attack fixtures in tests/epochs.test.ts, 89 tests/1551 assertions. Deferred with record: X7 published-surface freshness + traces.jsonl hardening + receipt.ts:158 X15-class (owner PAI, early W2). NEW EXECUTOR RULE: full gate rehearsal BEFORE declaring any epoch. Transpiler gotcha for the doctrine ledger: Bun silently elides statement-position declare() calls as TS ambient syntax.

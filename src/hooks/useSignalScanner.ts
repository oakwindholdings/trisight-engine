...
export function useSignalScanner(symbols: string[], timeframe: string, shouldScan: boolean = true): { signals: TradeActionSignal[]; isScanning: boolean } {
  ...
  useEffect(() => {
    ...
    const fetchAndProcess = async () => {
      setIsScanning(true);
      const intervalParam = mapTimeframeToInterval(timeframe);
      const allSignals: TradeActionSignal[] = [];
      const auditLog: any[] = [];

      const chunkSize = 250;
      const chunks = symbols.reduce((res: string[][], val, i) => {
      ...
      for (const batch of chunks) {
        try {
          ...
          const apiUrl = `https://api.twelvedata.com/time_series?symbol=${symbolParam}&interval=${intervalParam}&outputsize=30&apikey=${process.env.REACT_APP_TWELVE_DATA_API_KEY}`;
          ...
          for (const ticker of batch) {
            ...
            const patternResults = await detectPatternsForSymbol(ticker, ohlcv);
...
async function fetchOHLCVForSymbol(ticker: string, timeframe: string): Promise<CandlestickData[]> {
  const mappedTimeframe = mapTimeframeToInterval(timeframe);
  const interval = timeframeToInterval(mappedTimeframe);
  ...
  switch (timeframe) {
    case '1D':
      startDate.setDate(endDate.getDate() - 7); // 1 week of data
      break;
    case '1W':
      startDate.setDate(endDate.getDate() - 30); // 1 month of data
      break;
    case '1M':
      startDate.setDate(endDate.getDate() - 90); // 3 months of data
      break;
    default:
      startDate.setDate(endDate.getDate() - 30); // Default to 1 month
  }
  
  return await fetchCandlestickData(ticker, interval, startDate, endDate);
}

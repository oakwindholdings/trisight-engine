...
// Helper utilities for learning metrics
// Provides calculations for useLearning
...
export const calculateIncorporationRate = (patternType: PatternType): number => {
  return 0.75;
};
...
export const calculateRecall = (feedback: PatternFeedback[]): number => {
  return 0.8;
};
...
export const getParametersEvolution = (
  ...
  const allParams: Record<PatternType, PatternDetectionParameters> = {} as Record<
    PatternType,
    PatternDetectionParameters
  >;
...
export const computeLearningMetrics = (
  processor: LearningProcessor
): LearningMetrics => {
  ...
  Object.values(PatternType).forEach(patternType => {
    ...
    const improvementMetrics = FeedbackAggregator.calculateImprovementMetrics(
      allFeedback,
      patternType
    );
  ...
  return {
    ...
    accuracyByPatternType: Object.values(PatternType).reduce((acc, type) => {
      ...
      const falsePos = typeFeedback.filter(f => f.falsePositive).length;
      acc[type] = typeFeedback.length > 0 ? 1 - falsePos / typeFeedback.length : 0;
      return acc;
